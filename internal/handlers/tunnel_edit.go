...
func handleTunnelEdit(ctx *actions.Context) error {
	...
	case config.TransportNaive:
		if tunnel.Naive != nil {
			...
			portStr := ctx.GetArg("naive-port")
			if portStr == "" {
				var err error
				portStr, err = prompt.String("Port", fmt.Sprintf("%d", tunnel.Naive.ResolvedPort()))
				if err != nil {
					return err
				}
			}
			var newPort int
			if n, e := fmt.Sscanf(portStr, "%d", &newPort); n == 1 && e == nil && newPort > 0 && newPort != tunnel.Naive.Port {
				tunnel.Naive.Port = newPort
				changed = true
				out.Success(fmt.Sprintf("Port set to %d", newPort))
			}

			before := *tunnel.Naive
			if err := applyNaiveArgs(tunnel.Naive, ctx.Args); err != nil {
				return actions.NewError(actions.TunnelEdit, "invalid NaiveProxy options", err)
			}
			if !hasNaiveAdvancedArgs(ctx.Args) {
				if err := promptNaiveAdvanced(tunnel.Naive); err != nil {
					return err
				}
			}
			if before != *tunnel.Naive {
				changed = true
			}
		}
	}
